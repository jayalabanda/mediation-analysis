...
DATA_stremr <-  subset(data.sim, 
                       select = - c(Y_qol) ) # on exclue la variable d'outcome continu des données, car elle est inutile
names(DATA_stremr)
...
dataDT0 <- data.frame(ID = DATA_stremr$ID,
                      L0_male = DATA_stremr$L0_male,
                      L0_parent_low_educ_lv = DATA_stremr$L0_parent_low_educ_lv,
                      t = DATA_stremr$t0, 
                      L1  = rep(NA, nrow(DATA_stremr)), 
                      A.t = DATA_stremr$A0_ace,
                      Y.tplus1 = DATA_stremr$alive, # il sont tous vivants avant l'exposition à M_smoking,
                                                    # pas de censure dans cet exemple
                      A.tminus1 = rep(NA, nrow(DATA_stremr)))
                      
dataDT1 <- data.frame(ID = DATA_stremr$ID,
                      L0_male = DATA_stremr$L0_male,
                      L0_parent_low_educ_lv = DATA_stremr$L0_parent_low_educ_lv,
                      t = DATA_stremr$t1, 
                      L1  = DATA_stremr$L1, 
